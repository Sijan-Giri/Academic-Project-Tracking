    ...
    <div className="space-y-6 text-white min-h-screen p-6 bg-[#0f1117]">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Overview of institutional academic projects, users, and activity</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="bg-white/5 backdrop-blur-md border-white/10">
          <CardHeader><CardTitle className="text-white text-base font-semibold">Projects by Department</CardTitle></CardHeader>
          <CardContent className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={deptChartData}>
                <XAxis dataKey="name" stroke="#94a3b8" />
                <YAxis stroke="#94a3b8" />
                <Tooltip cursor={{ fill: 'rgba(255,255,255,0.05)' }} contentStyle={{ backgroundColor: '#1a1a2e', borderColor: 'rgba(255,255,255,0.1)', color: '#fff' }} />
        ...
        <Card className="bg-white/5 backdrop-blur-md border-white/10">
          <CardHeader><CardTitle className="text-white text-base font-semibold">User Role Distribution</CardTitle></CardHeader>
          <CardContent className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <Tooltip contentStyle={{ backgroundColor: '#1a1a2e', borderColor: 'rgba(255,255,255,0.1)', color: '#fff' }} />
      ...
      <Card className="bg-white/5 backdrop-blur-md border-white/10">
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="text-white text-base font-semibold">Recent Audit Logs</CardTitle>
          <Button variant="ghost" size="sm" className="text-indigo-400 hover:text-indigo-300" onClick={() => navigate('/admin/audit')} id="view-audit-btn">
        ...
        <CardContent>
          <Table>
            ...
            <TableBody>
              {logs.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={4} className="text-center py-6 text-gray-500">No recent audit log activity</TableCell>
                </TableRow>
              ) : (
                logs.slice(0, 5).map((log: any) => (
                  <TableRow key={log.id} className="border-white/5 hover:bg-white/5">
                    <TableCell className="text-gray-400 text-xs">{format(new Date(log.createdAt || Date.now()), 'MMM d, yyyy HH:mm:ss')}</TableCell>
                    <TableCell className="font-medium text-indigo-300">{log.user?.name || log.userId || 'System'}</TableCell>
                    <TableCell>
                      <span className="px-2 py-0.5 rounded text-xs bg-indigo-500/20 text-indigo-400 font-mono">{log.action}</span>
                    </TableCell>
                    <TableCell className="text-gray-300 capitalize text-xs">{log.entityType}</TableCell>
                  </TableRow>
                ))
              )}
            </TableBody>
