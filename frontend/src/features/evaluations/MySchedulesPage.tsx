    ...
    <div className="space-y-6 mt-6">
      {list.length === 0 ? (
        ...
        list.map((s: any) => {
          ...
          return (
            <Card key={s.id} className="overflow-hidden border border-border shadow-md hover:border-indigo-500/40 transition-all rounded-2xl bg-card">
                ...
                <div className="bg-secondary/40 px-6 py-4 border-b border-border flex flex-wrap items-center justify-between gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <span className={`px-3 py-1 rounded-full text-xs font-semibold border ${
                      isOnline ? 'bg-indigo-500/10 text-indigo-400 border-indigo-500/30' : 'bg-slate-500/10 text-slate-400 border-slate-500/30'
                    }`}>
                      {isOnline ? <Video className="w-3.5 h-3.5 inline mr-1 text-indigo-400" /> : <MapPin className="w-3.5 h-3.5 inline mr-1 text-slate-400" />}
                      {isOnline ? 'Online Presentation' : 'In-Person / Offline'}
                    </span>
                  </div>

                  {s.isCompleted ? (
                    <span className="px-3 py-1 rounded-full text-xs font-bold bg-emerald-50 dark:bg-emerald-500/15 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-500/30">
                      Completed
                    </span>
                  ) : (
                    <span className="px-3 py-1 rounded-full text-xs font-bold bg-amber-50 dark:bg-amber-500/15 text-amber-800 dark:text-amber-300 border border-amber-200 dark:border-amber-500/30">
                ...
                <div className="p-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="lg:col-span-2 space-y-4">
                    <div className="flex items-start gap-4">
                      <div className="bg-indigo-600/10 dark:bg-indigo-500/15 border border-indigo-500/20 rounded-xl p-3.5 text-center shrink-0 min-w-[76px]">
                        <span className="block text-2xl font-black text-indigo-600 dark:text-indigo-400 tracking-tight leading-none">{format(dateObj, 'dd')}</span>
                      ...
                      <div className="rounded-xl border border-indigo-500/30 bg-indigo-500/5 p-4 space-y-3">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-[11px] text-emerald-400 font-semibold flex items-center gap-1">
                            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" /> Active Link
                          </span>
                        </div>

                        <p className="text-xs text-slate-800 font-mono truncate bg-gray/10 p-2.5 rounded-lg border border-white/10 select-all">
                          {meetingUrl}
                        </p>
                        ...
                        <div className="flex flex-wrap items-center gap-3 pt-1">
                          <a
                            ...
                            className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-indigo-500 to-violet-600 hover:from-indigo-600 hover:to-violet-700 text-white font-semibold text-xs shadow-md shadow-indigo-500/20 transition-all"
                          >
                          ...
                          >
                            {copiedId === s.id ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5 text-indigo-400" />}
